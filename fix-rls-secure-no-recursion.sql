-- Fix RLS with secure policies that don't cause recursion
...
-- 3. Create simple, non-recursive policies for assigned_word_sets
-- Teachers can manage assignments for their own word sets
CREATE POLICY "assigned_word_sets_teacher_own" ON assigned_word_sets
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY "assigned_word_sets_student_own" ON assigned_word_sets
  FOR SELECT USING (
...
-- 4. Create simple, non-recursive policies for word_sets
-- Teachers can manage their own word sets
CREATE POLICY "word_sets_teacher_own" ON word_sets
  FOR ALL USING (teacher_id = auth.uid());

-- Students can view word sets that are assigned to them (no recursion)
CREATE POLICY "word_sets_student_assigned" ON word_sets
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM assigned_word_sets 
      WHERE assigned_word_sets.word_set_id = word_sets.id
      AND (
        assigned_word_sets.student_id = auth.uid() OR
        (assigned_word_sets.class_id IS NOT NULL AND EXISTS (
          SELECT 1 FROM class_students 
          WHERE class_students.student_id = auth.uid()
          AND class_students.class_id = assigned_word_sets.class_id
        ))
