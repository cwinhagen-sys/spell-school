...
export default function TypingChallengeLeaderboard({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-3xl p-6 w-full max-w-2xl shadow-2xl my-8 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between mb-6 flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">Typing Challenge Leaderboard</h2>
              <p className="text-sm text-gray-600">Top 10 Players</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {userRank && userRank <= 10 && userKpm !== undefined && userTime !== undefined && (
          <div className="mb-6 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl border-2 border-yellow-300 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-bold text-gray-800">🎉 Du är med i topplistan!</p>
                  <p className="text-sm text-gray-600">Rang #{userRank} • {userKpm} KPM • {formatTime(userTime)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Laddar leaderboard...</p>
              <p className="text-xs text-gray-500 mt-2">Väntar på att resultat registreras...</p>
            </div>
          ) : leaderboard.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-600">Inga resultat ännu. Var den första!</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="min-w-0">
                        <p className={`font-semibold truncate ${isUser ? 'text-orange-700' : 'text-gray-800'}`}>
                          {entry.username}
                          {isUser && ' (Du)'}
                        </p>
                    ...
                    <div className="flex items-center space-x-6 text-right flex-shrink-0">
                      <div>
                        <p className="text-sm text-gray-600">KPM</p>
                        <p className="font-bold text-gray-800">{entry.kpm}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Tid</p>
                        <p className="font-bold text-gray-800">{formatTime(entry.duration_sec)}</p>
                      </div>
                    </div>
                  </div>
                )
